...
create table
if not exists Privilegio
(
    idPrivilegio int not null auto_increment primary key,
    nombrePrivilegio varchar
(25) not null,
    descripcionPrivilegio varchar
(100)
);

create table
if not exists EquipoTrabajo
(
...
(50),
    prioridadIssue enum
('Alta', 'Media-Alta', 'Media', 'Media-Baja', 'Baja') not null default 'Baja',
    estadoIssue enum
('To Do', 'In Progress', 'Done') not null default 'To Do',
    fechaCreacion timestamp not null default current_timestamp,
    fechaFinalizacion timestamp not null default current_timestamp
);

create table
if not exists Retroalimentacion
(
...
create table
if not exists Accionable
(
    idAccionable int not null auto_increment primary key,
    nombreAccionable varchar
(50) null,
    storyPoints int default 0 not null,
    labelAccionable varchar
(50),
    prioridadAccionable enum
('Alta', 'Media-Alta', 'Media', 'Media-Baja', 'Baja') not null default 'Media',
    estadoAccionable enum
('Aprobado', 'No aprobado') not null default 'No aprobado',
    estadoIssue enum
('To Do', 'In Progress', 'Done') not null default 'To Do',
    fechaCreacion timestamp not null default current_timestamp ,
    fechaFinalizacion timestamp not null default current_timestamp
...
(idEmpleado),
    foreign key
(idRol) references Rol
(idRol)
);

create table
if not exists RolPrivilegio
(
    idRol int not null, 
...
(idRol),
    foreign key
(idPrivilegio) references Privilegio
(idPrivilegio)
);
...
create table
if not exists EquipoTrabajoIssue
(
    idEquipoTrabajo int not null,
...
(idEquipoTrabajo),
    foreign key
(idIssue) references Issue
(idIssue)
);

create table
if not exists SprintIssue
(
    idIssue int not null, 
    idSprint int not null, 
...
(idIssue),
    foreign key
(idSprint) references Sprint
(idSprint)
);

create table
if not exists SprintEpica
(
    idSprint int not null,
    idEpica int not null,
...
(idSprint),
    foreign key
(idEpica) references Epica
(idEpica)
);

create table
if not exists RetroalimentacionPregunta
(
    idRetroalimentacion int not null,
    idPregunta int not null,
    ...
    primary key
(idRetroalimentacion, idPregunta),
    foreign key
(idRetroalimentacion) references Retroalimentacion
(idRetroalimentacion),
    foreign key
(idPregunta) references Pregunta
(idPregunta)
);


create table
if not exists CualitativaAccionable
(
...
(idCualitativa, idAccionable),
    foreign key
(idCualitativa) references Cualitativa
(idCualitativa),
    foreign key
(idAccionable) references Accionable
(idAccionable)
);
...
,
add constraint fk_idReporte foreign key
(idReporte) references Reporte
(idReporte);
...
,
add constraint fk_idRetroalimentacionCuanti foreign key
(idRetroalimentacion) references RetroalimentacionPregunta
(idRetroalimentacion);
...
,
add constraint fk_idRetroalimentacionCuali foreign key
(idRetroalimentacion) references RetroalimentacionPregunta
(idRetroalimentacion);